...
if __name__ == '__main__':
	...
	print(time.time()-starttime, ":","clean data ready")
	...
	#find those train&test intui&btextual in that tfidf matrix
	train_textual_x = []
	train_intui_x = []
	...
	#training with SVM
	#combined with gridsearch
	params = {'kernel':('linear','poly', 'rbf'), 'degree':[3,4], 'C':[1, 2, 4], 'gamma':[0.125, 0.25, 0.5 ,1, 2, 4]}

	svm = SVC()
	clf = GridSearchCV(svm, params, n_jobs=-1)
	textual_training = clf.fit(train_textual_x, train_textual_y)
	textual_pred = clf.predict(test_textual_x)
	print(time.time()-starttime, ":","svm training for textual task done")
	print('The parameters of the best model are: ')
	print(clf.best_params_)

	intui_training = clf.fit(train_intui_x, train_intui_y)
	intui_pred = clf.predict(test_intui_x)
